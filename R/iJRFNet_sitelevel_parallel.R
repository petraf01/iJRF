...
# --- MAIN function
"iJRFNet_sitelevel_parallel" <-
  function(X, W=NULL,ntree=NULL,mtry=NULL,model=NULL,genes.name,ptm.name=NULL,parallel,site.name=NULL) {
    
     if (is.null(model)) {print("Error: Specify Model")} else { 
      if (is.null(ntree)) ntree=1000
      
      if (model=="iJRF")  out<-iJRF_sitelevel_parallel(X=X,W=W,ntree=ntree,mtry=mtry,genes.name=genes.name,parallel=parallel,site.name=site.name)
