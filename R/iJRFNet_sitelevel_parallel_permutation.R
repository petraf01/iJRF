...
#' Site level JRF - network connecting sites. It excludes connection between sites from the same gene
...
"iJRF_sitelevel_parallel_permutation" <-
  function(X, W=NULL,ntree=NULL,mtry=NULL,genes.name=NULL,parallel,seed,site.name) {
...
# --- MAIN function
"iJRFNet_sitelevel_parallel_permutation" <-
  function(X, W=NULL,ntree=NULL,mtry=NULL,model=NULL,genes.name,ptm.name=NULL,parallel,seed,site.name=NULL) {
    
     if (is.null(model)) {print("Error: Specify Model")} else { 
      if (is.null(ntree)) ntree=1000

      if (model=="iJRF")  out<-iJRF_sitelevel_parallel_permutation(X=X,W=W,ntree=ntree,mtry=mtry,genes.name=genes.name,parallel=parallel,seed=seed,site.name=site.name)
